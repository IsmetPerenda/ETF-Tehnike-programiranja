...
public:
	...
	friend std::ostream &operator <<(std::ostream &tok, const Kompleksni &a)
	{
		...
		if(a.re==0 && a.im==0) return tok << "0";
			...
			if(a.im!=1) return tok << a.im << "i";
			else return tok << "i";
		}
		else if(a.im==0) return tok << a.re;
		else{
			if(a.im==1) return tok << a.re << "+i";
			else if(a.im==-1) return tok << a.re << "-i";
			else if(a.im<0) return tok << a.re << a.im << "i";
			else if(a.im>0) return tok << a.re << "+" << a.im << "i";
		}
	}
...
int main ()
{
	std::cout << Kompleksni(0,5) << " " << Kompleksni(10) << std::endl;
	std::cout << Kompleksni(1,2) << " " << Kompleksni(0,1) << std::endl;
	return 0;
}