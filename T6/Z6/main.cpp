...
auto KreirajDinamickuKopiju2D(KontejnerskiTip v) -> typename std::remove_reference<decltype(v.at(0).at(0))>::type**{
	int brojac=0;
	for(int i=0; i<v.size(); i++){			//Brojimo koliko elemenata ima kontejner koji smo poslali u funkciju
	...
	try{
		...
		try{
			m[0]=new typename std::remove_reference<decltype(v.at(0).at(0))>::type[brojac];
			for(int i=1; i<v.size(); i++){
				m[i]=m[i-1]+v.at(i-1).size();
			}
